...
export function register(email, password, firstName, lastName, profilePictureURL, referredBy){
    return (dispatch) => {
        ...
        return axios.post('http://link-in-bio.limited/auth/register', { email, password, firstName, lastName, profilePictureURL, referredBy} )
        .then((res) => {
            ...
            return axios.post('http://link-in-bio.limited/l/new', {'userId':res.data.userId, 'backColor':'#ffffff','txtColor':'#000000', 'fontSelection':'Roboto',}, { headers: {authorization: res.data.token} })
            .then((res) => {
                ...
                return axios.post('http://link-in-bio.limited/e/new', { userId, listId, referencingURL, description, linkTitle, imgURL }, { headers: {authorization: token} })
                .then((res) => {
                    console.log('create newList Std Entry', res.data)
                    const useThisURL = `http://link-in-bio.limited/s/?eid=${res.data.result[0].entryId}&ref=${res.data.result[0].referencingURL}&red=f`
...
export function login(email, password, token){
    return (dispatch) => {
        ...
        return axios.post('http://link-in-bio.limited/auth/login', { email, password, token })
        .then(async (res) => {
            const payload = res.data
            // localStorage.setItem('token', res.data.token)
            // localStorage.setItem('userId', res.data.userId)
            ...
            const listIdHolder = await axios.get(`http://link-in-bio.limited/l/list4user/${res.data.userId}`)
...
export function createList(userId, backColor, txtColor, fontSelection, token){
    return (dispatch) => {
        ...
        return axios.post('http://link-in-bio.limited/l/new', { userId, backColor, txtColor, fontSelection }, { headers: {authorization: token} })
        .then((res) => {
...
export function addEntry(userId, listId, referencingURL, description, linkTitle, imgURL, token, shackImageId){
    return (dispatch) => {
        ...
        return axios.post('http://link-in-bio.limited/e/new', { userId, listId, referencingURL, description, linkTitle, imgURL, shackImageId }, { headers: {authorization: token} })
        .then((res) => {
            const useThisURL = `http://link-in-bio.limited/s/?eid=${res.data.result[0].entryId}&ref=${res.data.result[0].referencingURL}&red=f`
            return axios.get(useThisURL)
            .then((res) => {
...
export function editEntry(entryId, referencingURL, description, linkTitle, imgURL, token, listId){
    return (dispatch) => {
        ...
        return axios.put('http://link-in-bio.limited/e/replaceEntry', {entryId, referencingURL, description, linkTitle, imgURL, listId}, { headers: {authorization: token}})
        .then((res) => {
...
export function getListId(userId){
    return (dispatch) => {
        ...
        return axios.get(`http://link-in-bio.limited/l/${userId}`)
        .then((res) => {
