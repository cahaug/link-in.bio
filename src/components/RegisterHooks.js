        ...
        .then(async function(valid) {
            if (valid===true){
                try {
                    ...
                    const registrationReturn = await axios.post('http://link-in-bio.limited/auth/register', { email, password, firstName, lastName, profilePictureURL, referredBy} )
                    ...
                    const userListCreation = await axios.post('http://link-in-bio.limited/l/new', {'userId':registrationReturn.data.userId, 'backColor':'#ffffff','txtColor':'#000000', 'fontSelection':'Roboto',}, { headers: {authorization: registrationReturn.data.token} })
                    ...
                    const entryCreationReturnData = await axios.post('http://link-in-bio.limited/e/new', { userId, listId, referencingURL, description, linkTitle, imgURL }, { headers: {authorization: token} })
                    console.log('entryCreationReturnData', entryCreationReturnData)
                    // give the link its entry its first view
                    const useThisURL = `http://link-in-bio.limited/s/?eid=${entryCreationReturnData.data.result[0].entryId}&ref=${entryCreationReturnData.data.result[0].referencingURL}&red=f`
